...
class AdvancedTradingBot:
    def __init__(self, exchange_name: str = "kraken", api_key: str = None, 
                 api_secret: str = None, symbol: str = "BTC/USDT", 
        ...
        self.min_position_size = 0.50
        self.dynamic_stop_loss_base = 2.0
    ...
    def calculate_macd(self, prices):
        ...
        signal = self.calculate_ema(np.array([macd]), self.macd_signal)
    ...
    def calculate_support_resistance(self, df):
        ...
        recent = df['low'].tail(self.lookback_period).min()
        support = recent
        
        recent = df['high'].tail(self.lookback_period).max()
        resistance = recent
    ...
    def calculate_volatility(self, df):
        ...
        return max(0.01, min(volatility, 0.05))
    ...
    def execute_buy(self, current_price: float) -> bool:
        ...
        if self.account_type == "paper":
            ...
            self.trades_today += 1
            print(f"🟢 BUY: {amount:.6f} {self.symbol.split('/')[0]} @ ${current_price:.2f}")
            print(f"   Position size: {self.initial_position_size*100:.0f}% of balance")
        else:
            try:
    ...
    def execute_sell(self, current_price: float) -> bool:
        ...
        try:
            if self.account_type == "paper":
                ...
                print(f"🔴 SELL: {amount:.6f} {self.symbol.split('/')[0]} @ ${current_price:.2f}")
    ...
    def run_loop(self, interval_seconds: int = 60, max_iterations: Optional[int] = None):
        iteration = 0
        print(f"\n💎 Starting ADVANCED trading loop ({self.symbol})...")
        print(f"   Account type: {self.account_type.upper()}")
        print(f"   Strategy: Multi-Indicator + Risk Management")
    ...
    def print_summary(self):
        ...
        print("\n" + "="*60)
        print("💎 ADVANCED BOT - TRADING SUMMARY")
        print("="*60)
        ...
        print(f"Win rate: {self.calculate_win_rate():.1f}%")
        if self.current_position:
